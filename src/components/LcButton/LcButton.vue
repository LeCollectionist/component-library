...
const computedClass = computed(() => {
  ...
  const blockClass: ComputedClass = {
    ...
    icon: props.block ? 'lc-icon--block' : '',
  }
  ...
  return [
    ...
    btnColors[props.variant] && btnColors[props.variant][color] ? btnColors[props.variant][color] : '',
    btnSizes[props.variant] && btnSizes[props.variant][size] ? btnSizes[props.variant][size] : '',
    btnVariants[props.variant] ? btnVariants[props.variant] : '',
...
<template>
  <component :is="type" :class="computedClass" :disabled="disabled" v-bind="$attrs"
    :type="type === 'button' && !$attrs.type ? 'button' : $attrs.type" @click="callback">
    <span v-if="loader" class="lc-btn-loader" data-testid="lc-btn-loader">
